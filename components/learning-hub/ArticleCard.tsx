...
export default function ArticleCard({ article, featured = false }: { article: LearningArticle; featured?: boolean }) {
  return (
    <Link
      ...
      className={`group block h-full rounded-3xl border border-secondary/10 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md ${
        featured ? "p-7" : "p-6"
      }`}
    >
      ...
      <h3 className={`${featured ? "mt-5 text-3xl" : "mt-4 text-xl"} font-black tracking-tight text-secondary group-hover:text-primary`}>
        {article.title}
      </h3>
