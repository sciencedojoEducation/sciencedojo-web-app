...
import {
  submitTutorAcademyQuiz,
  type QuizActionState,
} from "@/app/dashboard/tutor/academy/actions";
...
export default function AcademyQuiz({
  ...
  if (state.status === "passed") {
    return (
      <section
        className="border-y border-[#DEDFE1] py-10 text-center"
        aria-live="polite"
      >
        <span className="mx-auto inline-flex h-16 w-16 items-center justify-center rounded-full bg-[#1E5AA8] text-white">
          <Trophy size={31} />
        </span>
        ...
        <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
          ...
          <Link
            href={courseHref}
            className="inline-flex min-h-11 items-center justify-center border border-[#C9CDD2] bg-white px-7 text-xs font-bold uppercase tracking-[0.1em] text-[#4A4B4E]"
          >
            Review course
          </Link>
        </div>
  ...
  const resultMap = new Map(
    state.results?.map((result) => [result.questionId, result]),
  );

  return (
    <form action={formAction} className="space-y-5">
      {previousBestScore > 0 && (
        <div className="border-l-4 border-[#1E5AA8] bg-[#F1F6FC] px-5 py-4 text-sm font-semibold text-[#173A63]">
          Your best score so far is {previousBestScore}%.
        </div>
      )}
      {state.message && (
        <div
          role="alert"
          className={`border-l-4 px-5 py-4 text-sm font-semibold ${state.status === "error" ? "border-red-700 bg-red-50 text-red-900" : "border-[#C4943F] bg-[#FBF7EE] text-[#59451F]"}`}
        >
          {state.message}
        </div>
      )}
      ...
      {questions.map((question, index) => {
        ...
        return (
          ...
          >
            ...
            {result && (
              <div
                className={`mt-4 flex items-start gap-3 border-l-4 px-4 py-3 text-sm font-semibold leading-6 ${result.correct ? "border-[#438A7E] bg-[#F2F8F7] text-[#244743]" : "border-red-700 bg-red-50 text-red-900"}`}
              >
                {result.correct ? (
                  <CheckCircle2
                    size={19}
      ...
      <button
        type="submit"
        disabled={pending}
        className="inline-flex min-h-12 w-full items-center justify-center gap-2 bg-[#1E5AA8] px-7 text-sm font-bold uppercase tracking-[0.1em] text-white hover:bg-[#174A8B] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#1E5AA8] disabled:cursor-wait disabled:opacity-60"
      >
        {state.status === "failed" ? (
          <RotateCcw size={18} />
        ) : (
