      ...
      {sections.map((section) => (
        <details key={section} open className="group border-b border-[#DEDFE1]">
          <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between px-5 text-[10px] font-bold uppercase tracking-[0.16em] text-[#717376] outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#1E5AA8] [&::-webkit-details-marker]:hidden">
            {section}
            <ChevronDown size={14} className="transition-transform group-open:rotate-180 motion-reduce:transition-none" aria-hidden="true" />
          </summary>
          <ol>
  ...
  return (
    <aside className="hidden h-full w-[280px] shrink-0 overflow-y-auto border-r border-[#DEDFE1] bg-white lg:block">
      ...
      <div className="border-b border-[#DEDFE1] px-5 py-4">
        ...
        <div className="mt-3 h-1 overflow-hidden bg-[#E6E7E9]" aria-label={`${progressPercent}% course progress`} role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progressPercent}>
...
export default function AcademyCourseNavigation({ children, course, basePath, progress: initialProgress }: { children: React.ReactNode; course: NavigationCourse; basePath: string; progress: AcademyProgress }) {
  ...
  useEffect(() => {
    ...
    const focusable = drawerRef.current?.querySelectorAll<HTMLElement>('a[href], button:not([disabled]), summary, [tabindex]:not([tabindex="-1"])');
    focusable?.[0]?.focus();
