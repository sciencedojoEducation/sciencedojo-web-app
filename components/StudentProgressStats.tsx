...
export default function StudentProgressStats({ bookings, mobileDensity = "default", currentFocus }: StudentProgressStatsProps) {
  ...
  // Highlight recent learning rhythm when a completed lesson happened recently.
  let isActive = false;
  ...
  return (
    <section>
      <div className={isCompact ? "mb-3 md:mb-6" : "mb-4 md:mb-6"}>
        <p className="text-[10px] font-black uppercase tracking-[0.16em] text-primary/65">Progress overview</p>
        <h2 className="mt-2 text-lg font-black text-secondary md:text-xl">How is learning developing?</h2>
        <p className={`mt-1.5 max-w-2xl text-sm font-medium text-secondary/50 md:mt-2 md:leading-7 ${isCompact ? "leading-6" : "leading-7"}`}>
          A parent-friendly view of lessons, time, subjects, and the current learning focus.
        </p>
      </div>
      <div className={`mb-2 grid grid-cols-2 md:grid-cols-4 md:gap-4 ${isCompact ? "gap-2.5" : "gap-3"}`}>
        ...
        <div className={`flex flex-col items-center justify-center rounded-2xl border border-secondary/10 bg-white text-center shadow-sm md:rounded-3xl md:p-6 ${isCompact ? "p-3" : "p-4"}`}>
          ...
          <div className="text-[9px] font-black uppercase tracking-[0.12em] text-secondary/40 md:text-[10px] md:tracking-widest">Lessons completed</div>
        </div>
        ...
        <div className={`flex flex-col items-center justify-center rounded-2xl border border-secondary/10 bg-white text-center shadow-sm md:rounded-3xl md:p-6 ${isCompact ? "p-3" : "p-4"}`}>
          ...
          <div className="text-[9px] font-black uppercase tracking-[0.12em] text-secondary/40 md:text-[10px] md:tracking-widest">Learning hours</div>
        </div>
        ...
        <div className={`flex flex-col items-center justify-center rounded-2xl border border-secondary/10 bg-white text-center shadow-sm md:rounded-3xl md:p-6 ${isCompact ? "p-3" : "p-4"}`}>
          ...
          <div className="text-[9px] font-black uppercase tracking-[0.12em] text-secondary/40 md:text-[10px] md:tracking-widest">Subjects supported</div>
        </div>
        ...
        <div className={`${isCompact ? "p-3" : "p-4"} flex flex-col items-center justify-center rounded-2xl border text-center shadow-sm md:rounded-3xl md:p-6 ${isActive ? 'bg-gradient-to-br from-green-400 to-green-600 border-green-500 text-white' : 'bg-white border-secondary/10'}`}>
          ...
          <div className={`${isCompact ? "text-base" : "text-lg"} line-clamp-2 font-black leading-tight md:text-xl ${isActive ? 'text-white' : 'text-secondary'}`}>{currentFocus || (isActive ? 'Active' : 'Soon')}</div>
          <div className={`text-[9px] font-black uppercase tracking-[0.12em] md:text-[10px] md:tracking-widest ${isActive ? 'text-white/80' : 'text-secondary/40'}`}>Current focus</div>
        </div>
