  ...
  return (
    <div className="md:hidden">
      ...
      <label
        ...
        className="relative z-[70] inline-flex h-10 w-10 cursor-pointer touch-manipulation items-center justify-center rounded-xl border border-secondary/10 bg-white text-secondary shadow-sm transition-colors hover:bg-primary/5 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 md:hidden"
      >
        <span className="sr-only">Open menu</span>
        <span className="relative flex h-3.5 w-5 flex-col justify-between" aria-hidden="true">
          <span className={`h-0.5 rounded-full bg-current transition-transform ${isOpen ? "translate-y-[7px] rotate-45" : ""}`}></span>
          <span className={`h-0.5 rounded-full bg-current transition-opacity ${isOpen ? "opacity-0" : "opacity-100"}`}></span>
          <span className={`h-0.5 rounded-full bg-current transition-transform ${isOpen ? "-translate-y-[7px] -rotate-45" : ""}`}></span>
        </span>
      </label>

      <div
        className="fixed bottom-0 left-0 right-0 top-20 z-[45] hidden h-[calc(100dvh-5rem)] bg-secondary/35 backdrop-blur-sm peer-checked:block"
        onPointerDown={(event) => {
