-- Per-user dashboard badge view state.
...
DROP POLICY IF EXISTS "Users read own dashboard badge views" ON public.dashboard_badge_views;
CREATE POLICY "Users read own dashboard badge views"
  ON public.dashboard_badge_views FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users create own dashboard badge views" ON public.dashboard_badge_views;
CREATE POLICY "Users create own dashboard badge views"
  ON public.dashboard_badge_views FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users update own dashboard badge views" ON public.dashboard_badge_views;
CREATE POLICY "Users update own dashboard badge views"
  ON public.dashboard_badge_views FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- These queues can become actionable after their original creation time.
...
DROP TRIGGER IF EXISTS student_missions_touch_updated_at ON public.student_missions;
