...
import ClassSubjectArtwork from "@/components/ClassSubjectArtwork";
...
export default function ClassDetailUI({ classRoom, posts: initialPosts, bookings, isTutor, currentUserId }: ClassDetailUIProps) {
  ...
  const [activeTab, setActiveTab] = useState<"stream" | "assignments" | "sessions" | "missions">("stream");
  ...
  const currentTheme = getClassSubjectTheme(classRoom.subject, isStudent ? undefined : currentCoverColor);
  ...
  return (
    <div className={isStudent ? "min-h-screen bg-[#F6F8FC]" : "min-h-screen bg-background"}>
      {/* 📁 Archived Banner */}
      {classRoom.is_archived && (
         <div className={isStudent ? "border-b border-amber-200 bg-amber-50 px-4 py-3 text-center" : "bg-amber-400 py-3 text-center border-b border-amber-500 shadow-sm relative z-50"}>
            <p className={isStudent ? "flex items-center justify-center gap-2 text-sm font-medium text-amber-900" : "text-amber-950 text-xs font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3"}>
      ...
      {/* Dynamic Header */}
      <div 
        className={isStudent ? "relative isolate min-h-[165px] overflow-hidden p-4 sm:min-h-[190px] sm:p-6 md:min-h-[205px] md:p-8" : "relative flex min-h-[21rem] items-end p-4 transition-colors duration-500 md:h-64 md:min-h-0 md:p-8"}
        style={isStudent ? { backgroundColor: currentTheme.bannerColor } : { background: currentTheme.gradient }}
      >
        {isStudent && <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-[46%] text-white opacity-75 sm:block"><ClassSubjectArtwork kind={currentTheme.artwork} /></div>}
        {!isStudent && <div className="absolute inset-0 opacity-20 flex justify-center items-center overflow-hidden">
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="min-w-0 flex-1 md:pr-6">
            <Link href="/dashboard/classes" className={isStudent ? "mb-3 inline-flex min-h-11 items-center gap-2 rounded-md px-2 text-sm font-medium text-white hover:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white" : "mb-4 inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-white/70 transition-colors hover:text-white"}>
            ...
            {isEditingName ? (
              ...
              <div className="flex items-center gap-3 group">
                 <h1 className={isStudent ? "max-w-2xl break-words text-2xl font-semibold tracking-tight text-white sm:max-w-[55%] md:text-3xl lg:max-w-[65%]" : "max-w-2xl truncate text-4xl font-black tracking-tight text-white drop-shadow-md text-shadow-lg md:text-5xl"}>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 md:gap-4">
              <p className={isStudent ? "flex items-center gap-3 text-sm text-white" : "flex items-center gap-3 text-lg font-bold text-white"}>
                <span className={isStudent ? "inline-flex items-center gap-2 rounded-md border border-white/25 bg-white/15 px-3 py-1 font-medium text-white" : "rounded-lg border border-white/20 bg-white/20 px-3 py-1 text-xs uppercase tracking-widest backdrop-blur-md"}>{currentTheme.label}</span>
              </p>
          ...
          {!isStudent && <div className="relative flex w-fit max-w-full items-center gap-3 rounded-2xl border border-white/20 bg-white/10 p-3 shadow-lg backdrop-blur-md md:bottom-[-1rem] md:gap-4">
             <div className="h-12 w-12 flex-shrink-0 overflow-hidden rounded-xl border-2 border-white/50 bg-white/10 shadow-sm md:h-14 md:w-14">
                {otherAvatar ? (
                  ...
                  <div className={isStudent ? "flex h-full w-full items-center justify-center bg-slate-200 text-sm font-semibold text-slate-800" : "w-full h-full flex items-center justify-center text-white font-black text-lg"}>
                    {(otherName || "U").charAt(0)}
                  </div>
                )}
             </div>
             <div className="min-w-0 pr-1 md:pr-2">
                <p className={isStudent ? "mb-0.5 text-xs text-slate-600" : "text-xs text-white/60 font-black uppercase tracking-widest mb-0.5"}>{isStudent ? "Tutor" : "Student"}</p>
                <p className={isStudent ? "truncate text-sm font-semibold text-slate-900" : "truncate text-sm font-bold text-white md:whitespace-nowrap"}>{otherName}</p>
             </div>
          </div>}
        </div>
      </div>

      {isStudent && <div className="border-b border-slate-200 bg-white px-4 py-3 sm:px-6 md:px-8"><div className="mx-auto flex max-w-5xl items-center gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-100 text-sm font-semibold text-slate-700">
          {otherAvatar ? <img src={otherAvatar} alt="" className="h-full w-full object-cover" /> : (otherName || "U").charAt(0)}
        </div>
        <div className="min-w-0"><p className="text-xs text-slate-600">Your tutor</p><p className="truncate text-sm font-semibold text-slate-900">{otherName}</p></div>
      </div></div>}
      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-5 px-3 py-5 sm:px-4 md:px-8 md:py-8 lg:flex-row lg:gap-8">
        ...
        <div className="flex w-full flex-shrink-0 flex-col gap-4 lg:w-64 lg:gap-6">
           <div role="group" aria-label="Class sections" className={isStudent ? "no-scrollbar flex gap-1 overflow-x-auto rounded-xl border border-slate-200 bg-white p-1 lg:flex-col" : "no-scrollbar flex gap-1 overflow-x-auto rounded-[1.5rem] border border-secondary/10 bg-white p-2 shadow-sm shadow-black/5 lg:flex-col lg:rounded-[2rem] lg:shadow-lg"}>
              {[
                ...
                { id: "missions", label: "Missions", emoji: "🚀"}
                 ...
                 <button 
                   ...
                   className={isStudent ? `flex min-h-11 shrink-0 items-center rounded-lg px-4 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4f53a5] lg:w-full ${activeTab === tab.id ? "bg-[#eeefff] text-[#4f53a5]" : "text-slate-600 hover:bg-slate-50"}` : `flex shrink-0 items-center gap-2 rounded-2xl px-4 py-3 text-sm font-black transition-all lg:w-full lg:gap-3 lg:px-6 lg:py-4 ${
                     activeTab === tab.id 
                 ...
                 >
                   {!isStudent && <span className="text-xl">{tab.emoji}</span>}
                   {tab.label}
                 </button>
              ))}
           </div>
           ...
           {/* Upcoming Mini Card */}
           <div className={isStudent ? "rounded-xl border border-slate-200 bg-white p-5" : "group relative overflow-hidden rounded-[1.5rem] border border-secondary/10 bg-white p-4 shadow-sm lg:rounded-[2rem] lg:p-6"}>
              {!isStudent && <div className="absolute top-0 right-0 w-16 h-16 bg-primary/5 rounded-bl-[2rem] group-hover:scale-125 transition-transform"></div>}
              
              {!classRoom.is_archived && (
                 <button 
                   onClick={startLiveClass}
                   disabled={isSettingUpMeeting}
                   className={isStudent ? "mb-5 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#1E5AA8] px-4 text-sm font-semibold text-white hover:bg-[#174a8b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4f53a5] focus-visible:ring-offset-2 disabled:opacity-50" : "mb-5 flex w-full items-center justify-center gap-3 rounded-2xl bg-blue-600 p-3 text-xs font-black uppercase tracking-widest text-white shadow-md shadow-blue-200 transition-all hover:bg-blue-700 disabled:opacity-50 lg:mb-6 lg:p-4 lg:shadow-lg"}
                 >
