...
import { getAdminAcademyReviews } from "@/lib/academy-review";
...
export default async function EditAcademyCoursePage({
  ...
  if (!record) notFound();
  const [mediaLibrary, useV2, snapshots, reviews] = await Promise.all([
    ...
    record.id ? getAdminAcademyReviews(record.id) : Promise.resolve({ invitations: [], comments: [] }),
  ]);
  const course = { ...record.draft, id: record.id || undefined };
  if (useV2) return <AcademyCourseEditor initialCourse={course} status={record.status} mediaLibrary={mediaLibrary} initialRevision={record.draftRevision} snapshots={snapshots} reviewInvitations={reviews.invitations} reviewComments={reviews.comments} initialLessonId={query.lesson} initialAssessmentOpen={query.assessment === "true"} />;
