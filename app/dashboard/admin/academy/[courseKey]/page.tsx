...
export default async function EditAcademyCoursePage({ params }: { params: Promise<{ courseKey: string }> }) {
  ...
  if (useV2) return <AcademyCourseEditor initialCourse={course} status={record.status} mediaLibrary={mediaLibrary} initialRevision={record.draftRevision} snapshots={snapshots} />;
