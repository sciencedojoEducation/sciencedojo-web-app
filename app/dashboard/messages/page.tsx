...
export default async function MessagesPage({
  ...
  return (
    <div className={`h-full min-h-0 overflow-hidden lg:grid lg:grid-cols-[360px_1fr] ${isStudent ? "bg-[#F6F8FC]" : "bg-slate-50"}`}>
      ...
      <div className={`${activeConversation ? "block" : "hidden lg:block"} h-full min-w-0`}>
        {activeConversation ? (
          ...
          <div className={`hidden h-full flex-col items-center justify-center p-8 text-center lg:flex ${isStudent ? "bg-[#F6F8FC]" : "bg-slate-50"}`}>
             ...
             <h2 className={isStudent ? "mb-2 text-xl font-semibold text-slate-900" : "text-xl font-black text-secondary mb-2"}>{isStudent ? "Choose a conversation" : "Your Conversations"}</h2>
             <p className={isStudent ? "max-w-xs text-sm leading-6 text-slate-600" : "text-sm text-secondary/40 font-medium max-w-xs leading-relaxed"}>
                {isInternal ? "Select a teammate or admin to continue the conversation." : isStudent ? "Select a tutor conversation to read or send a message." : "Select a chat from the sidebar to start messaging your tutors or students."}
             </p>
          </div>
        )}
      </div>
