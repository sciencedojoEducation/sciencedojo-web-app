...
import {
  getEligibleAcademyCourses,
  getPublishedAcademyCourse,
...
export default async function TutorAcademyWelcomePage() {
  ...
  const course =
    (await getPublishedAcademyCourse(tutorAcademyCourse.key)) ||
    tutorAcademyCourse;
  const additionalCourses = (await getEligibleAcademyCourses()).filter(
  ...
  const hasStarted =
    progress.startedLessons.length > 0 ||
    progress.completedLessons.length > 0 ||
    Boolean(progress.currentLesson);
    ...
    <div className="min-h-full bg-white">
      <section className="relative flex min-h-[460px] items-end overflow-hidden bg-slate-900 sm:min-h-[550px]">
        <Image
          ...
          alt="A professional online tutor teaching from her workspace"
          fill
          priority
          sizes="100vw"
        ...
        <div className="mx-auto max-w-[600px]">
          ...
          <div className="mt-8 flex flex-wrap gap-x-6 gap-y-2 border-y border-[#DEDFE1] py-4 text-[12px] font-bold text-[#717376]">
            <span className="inline-flex items-center gap-2">
              <BookOpen size={15} aria-hidden="true" /> {course.lessons.length}{" "}
              lessons
            </span>
            <span className="inline-flex items-center gap-2">
          ...
          <section className="mt-14" aria-labelledby="course-contents-heading">
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#717376]">
                  Course contents
                </p>
                <h2
                  id="course-contents-heading"
                  className="mt-2 text-[30px] font-bold leading-10 tracking-[-0.02em] text-[#101010]"
                >
                  What you will learn
                </h2>
              </div>
              <span className="pb-1 text-sm font-bold text-[#1E5AA8]">
                {progressPercent}%
              </span>
            </div>
            <div
                  ...
                  <ol className="border-b border-[#DEDFE1]">
                    {course.lessons
                      ...
                      .map((lesson) => (
                        <li key={lesson.slug}>
                          <Link
                            href={`/dashboard/tutor/academy/lessons/${lesson.slug}`}
                            className="group flex min-h-14 items-center gap-4 border-t border-[#DEDFE1] py-3 outline-none focus-visible:ring-2 focus-visible:ring-[#1E5AA8] focus-visible:ring-offset-2"
                          >
                            <BookOpen
                              size={16}
                              strokeWidth={1.7}
                            ...
                            <span className="min-w-0 flex-1">
                              <span className="block text-[14px] font-bold leading-5 text-[#252629] transition-colors group-hover:text-[#1E5AA8]">
                                {lesson.title}
                              </span>
              ...
              {course.rules?.requireFinalAssessment !== false ? (
                <section>
                  ...
                  <Link
                    ...
                    className="group flex min-h-14 items-center gap-4 border-y border-[#DEDFE1] py-3 outline-none focus-visible:ring-2 focus-visible:ring-[#1E5AA8] focus-visible:ring-offset-2"
                  >
                    <BookOpen
                      size={16}
                      strokeWidth={1.7}
                      ...
                      aria-hidden="true"
                    />
                    <span className="min-w-0 flex-1 text-[14px] font-bold text-[#252629] transition-colors group-hover:text-[#1E5AA8]">
          ...
          {additionalCourses.length ? (
            <section className="mt-16 border-t border-[#DEDFE1] pt-12">
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#717376]">
                More Academy courses
              </p>
              <h2 className="mt-2 text-[30px] font-bold leading-10 tracking-[-0.02em] text-[#101010]">
                Continue learning
              </h2>
              <div className="mt-6 border-b border-[#DEDFE1]">
                {additionalCourses.map((item) => (
                  <Link
                    key={item.key}
                    href={`/dashboard/tutor/academy/courses/${item.key}`}
                    className="group flex min-h-20 items-center gap-4 border-t border-[#DEDFE1] py-4"
                  >
                    <BookOpen size={18} className="text-[#717376]" />
                    <span className="flex-1">
                      <strong className="block text-sm text-[#252629] group-hover:text-[#1E5AA8]">
                        {item.title}
                      </strong>
