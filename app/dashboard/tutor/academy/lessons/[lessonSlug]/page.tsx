...
export default async function TutorAcademyLessonPage({
  params,
  searchParams,
}: {
  ...
  if (
    ...
    redirect(`/dashboard/tutor/academy/lessons/${previousLesson.slug}`);
  }
  const completed =
      ...
      <div className="px-6 py-12 sm:px-10 sm:py-16">
        <div className="mx-auto max-w-[728px]">
          {error === "progress" && (
            <div
              role="alert"
              className="mb-8 border-l-4 border-red-700 bg-red-50 px-5 py-4 text-sm font-semibold leading-6 text-red-900"
            >
              Progress could not be saved. Confirm that the Tutor Academy
              database migration has been applied, then try again.
            </div>
          )}
          {error === "interactions" && (
            <div
              role="alert"
              className="mb-8 border-l-4 border-amber-600 bg-amber-50 px-5 py-4 text-sm font-semibold leading-6 text-amber-950"
            >
          ...
          <footer className="mt-16 border-t border-[#DEDFE1] pt-7">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              {previousLesson ? (
                <Link
                  href={`/dashboard/tutor/academy/lessons/${previousLesson.slug}`}
                  className="inline-flex min-h-11 items-center justify-center gap-2 border border-[#C9CDD2] bg-white px-6 text-xs font-bold uppercase tracking-[0.1em] text-[#4A4B4E] hover:border-[#1E5AA8] hover:text-[#1E5AA8] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#1E5AA8]"
                >
                  <ArrowLeft size={16} /> Previous
                </Link>
              ) : (
                <Link
                  href="/dashboard/tutor/academy"
                  className="inline-flex min-h-11 items-center justify-center gap-2 border border-[#C9CDD2] bg-white px-6 text-xs font-bold uppercase tracking-[0.1em] text-[#4A4B4E]"
                >
                  <ArrowLeft size={16} /> Course home
                </Link>
              )}
